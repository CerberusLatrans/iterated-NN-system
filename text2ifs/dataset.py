...
class IFSDataset(Dataset):
    def __init__(self,
        ...
        self.files = [f for f in os.listdir(self.json_dir) if not f.startswith(".")]
        self.encodings = []
        self.ifs_list = []
        for f in self.files:
            text = f.split(".")[0]
            encoding = self._encode_text(text)
            ifs = self._fetch_ifs(f)
            self.encodings.append(encoding)
            self.ifs_list.append(torch.tensor(ifs).float())
    
    def __len__(self):
        return len(self.files)
    
    def __getitem__(self, index):
        encoding_tensor = torch.tensor(self.encodings[index]).float()
        
        if self.scramble_ifs:
            ifs_tensor = torch.tensor(self.ifs_list[index].flatten()).float()
        else:
            ifs_tensor = torch.tensor(np.random.permutation(self.ifs_list[index]).flatten()).float()

        return encoding_tensor, ifs_tensor
